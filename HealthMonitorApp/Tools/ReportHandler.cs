...
public class ReportHandler(RepositoryService repositoryService)
{
    ...
    {
        string endpointJson = await repositoryService.ExtractControllersAndEndpointsAsJsonAsync(repositoryAnalysis);
        ...
        string reportPath = repositoryAnalysis.GetReportPath();
    ...
    private void RemoveNavbarItems(HtmlDocument document, string repositoryName)
    {
        ...
        if (titleNode != null)
        {
            titleNode.InnerHtml = $"Analysis Report for {repositoryName}";
        }
        ...
        var githubLink = document.DocumentNode.SelectSingleNode("//a[@href='https://github.com/Microsoft/ApplicationInspector']");
        githubLink?.ParentNode?.Remove();

        // Identify and remove the 'About' link
        var aboutLink = document.DocumentNode.SelectSingleNode("//a[@href='https://github.com/Microsoft/ApplicationInspector/wiki']");
        ...
        if (navbarBrand != null)
        {
            navbarBrand.InnerHtml = repositoryName;
        }
        ...
        if (overviewSection != null)
        {
            ...
            foreach (var apiGroup in apiGroups)
            {
                ...
                if (apiGroup.Annotations != null && apiGroup.Annotations.Any())
                {
                    ...
                    endpointOverview.Append($"<li style='margin-left: 20px; color:black;'>{joinedControllerAnnotations}</li>");
                }
                ...
                foreach (var endpoint in apiGroup.ApiEndpoints)
                {
                    ...
                    if (endpoint.Annotations != null && endpoint.Annotations.Any())
                    {
                        ...
                        endpointOverview.Append($"<li style='margin-left: 20px; color:black ;'>{joinedApiAnnotations}</li>");
                        endpointOverview.Append("</ul>");
                    }
    ...
    private void AddEndpointSummary(List<ApiGroup> apiGroups, HtmlDocument document)
    {
        ...
        if (overviewSection != null)
        {
            ...
            foreach (var apiGroup in apiGroups)
            {
                var controllerColor = apiGroup.IsAuthorized != null && apiGroup.IsAuthorized.Value ? "list-group-item-danger" : "list-group-item-success";
                ...
                endpointOverview.Append($"<a href='#' class='list-group-item {controllerColor} list-group-item-action'><i class='fas fa-cogs'></i> {apiGroup.Name}</a>");

                foreach (var endpoint in apiGroup.ApiEndpoints)
                {
                    var endpointColor = endpoint.IsOpen != null && endpoint.IsOpen.Value ? "list-group-item-success" : "list-group-item-danger";
                    ...
                    endpointOverview.Append($"<a href='#' class='list-group-item {endpointColor} list-group-item-action ml-3'><i class='fas fa-link'></i> {endpoint.Name}</a>");
                }
            }
