...
{
    private readonly Logger<DynamicStringProcessor> _logger = new(new LoggerFactory());
    ...
    private string ApplyVariables(ApiEndpoint apiEndpoint)
    {
        ...
        if (curlCommand != null)
        {
            ...
            foreach (Match match in matches)
            {
                ...
                var variableValue = repositoryAnalysisVariables.FirstOrDefault(rlv => rlv.Name == variableName)
                                        ?.DecryptVariable() ??
                                    apiGroupVariables.FirstOrDefault(agv => agv.Name == variableName)
                                        ?.DecryptVariable();
                curlCommand =
                    curlCommand.Replace(match.Value,
                        variableValue); // replace variable with the value in the original string
            }
        }
